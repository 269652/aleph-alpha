...
| Tile Building/Destruction System | ✅ Done | `Chunk.modifications` is now real: `EarthChunkManager.build_at_global`/`destroy_at_global` write to it, `TerrainRenderer.paint()` renders a modified cell over its generated biome tile, and `ChunkSerializer.save_modifications`/`load_modifications` persist it to `user://chunk_modifications/` across chunk unload/reload (walking away and back — this project's real equivalent of "save/reload", per the pre-existing plan noted in `EarthChunkManager.update()`'s doc comment). Player-facing: E turns the faced tile into bare earth (`TerrainRenderer.EARTH_TILE_ID`, Terraria-style terraforming -- E intentionally replaces whatever biome tile is there, grass included, per `Player._build_step()`'s doc comment), Q destroys/reverts it, on whichever tile the player is facing (`src/gameplay/tile_targeting.gd`). One live placeable tile type only (bare earth), no UI/inventory cost to building yet. `TerrainRenderer` is now ready to render two more modification tile ids with dedicated procedural art (not the flat earth-brown fill) — `atlas_coords_for_modification` looks up a per-structure atlas slot for `"campfire"`/`"furnace"` (`ProceduralStructureSprite`: a flame-on-embers tile and a brick-block-with-firebox tile, each 16x16, distinct from earth and from each other, tested) and falls back to the plain-earth slot for anything else — but nothing in live play writes those ids into `Chunk.modifications` yet; placing a carried `"placeable"`-kind item (item_catalog.gd already defines campfire/furnace as kind "placeable", and `hotbar_action.gd`'s `HotbarAction.PLACE` dispatch exists) into the world is a separate not-yet-landed mechanic. | large |
