...
gameplay), and the new Builder's own named limitations (no live spawner
yet decides a Builder should exist for a given project and injects its
piece layout; roof pieces out of scope; terrain buildability is a
permissive stand-in). `VillageRenderer._stamp_house` is still not migrated
to seed a `ConstructionProject` (deliberately out of scope this pass too)
— a settlement still gets its houses stamped for free at generation time,
not built up piece by piece by a real Builder over real elapsed time.
