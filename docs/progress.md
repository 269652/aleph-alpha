...
- **Aquatic ecosystem population simulation** (large) — ⬜ Not started —
  concept doc now fully specs the model: aggregate per-chunk
  `fish_population`/`fish_capacity` (water-area cell count × temperature
  suitability), seeded at equilibrium the first time a chunk loads, logistic
  growth + migration via the existing `population_model.gd`, an explicit
  fishing-harvest mortality term the land model still lacks, individual-
  fidelity promotion replacing `FishRenderer`'s independent per-cell roll,
  catch-up integration via `chunk_ecology_catchup.gd`, and real
  cross-session persistence via a new `ChunkSerializer.save_fish_population`/
  `load_fish_population` pair — see
  [fishing.md](concept/fishing.md#aquatic-population-model). No code changes
  yet.
- **Aquatic environmental factors** (medium) — ⬜ Not started — water
  temperature (as a capacity multiplier) is now spec'd, reusing existing
  `Chunk.temperature`; water quality/food-density remain open, see
