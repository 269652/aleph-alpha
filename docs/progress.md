...
- **Humanoid rig fix: the "double legs" hem** — ✅ Done — reported/screenshotted as "no arms, and double legs". The arms half had already been independently fixed in a concurrent pass (see `ARM_SWAY_AMPLITUDE`/the "no hands are visible" note above) by the time this landed; this pass's own contribution is the hem: `generate_hero_tunic_image`'s hem flared to 96% of shoulder width (`_HEM_FRACTION`), which worked out — via the real geometry against `CharacterView.LEG_SIZE`/leg positions — wider than the two legs' own combined span and directly overlapping them; for any class whose leg color sits close to its tunic/trim (e.g. "guard"), the hem read as an undifferentiated second pair of legs. Fixed by narrowing `_HEM_FRACTION` to continue tapering past the waist instead of flaring back out, pinned by a test that measures the real generated art against the real leg geometry (`test_tunic_hem_does_not_read_as_a_second_pair_of_legs` in `test_character_view.gd`) rather than eyeballing the constant. `test_hero_sprite.gd`'s one pre-existing assertion expecting the old flare was flipped to match.
