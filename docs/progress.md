...
EVERY real Storage in range, not just the single nearest one — closed
2026-08-25), and `ConstructionPriority.decide` now calls the real
`NeedResolver`. See the concept doc's own Status section for the full
accounting, including what's still not wired (no settlement-decision
