...
- **Humanoid rig fix: arms and the "double legs" hem** — ✅ Done — reported/screenshotted as "no arms, and double legs". Both confirmed by rendering `CharacterView` off-screen: (1) arms were only ever set visible while swimming (`_arm_left/right.visible` toggled off everywhere else in `_process`) — a walking/standing character rendered with a literally armless torso. Fixed: arms are visible in every movement state now, with a walking counter-swing (`arm_swing_offset`, reusing `LEG_SWING_AMPLITUDE` as its amplitude rather than a new number) alongside the existing swim stroke. (2) The tunic's hem flared to 96% of shoulder width (`_HEM_FRACTION`), which worked out — via the real geometry against `CharacterView.LEG_SIZE`/leg positions — wider than the two legs' own combined span and directly overlapping them; for any class whose leg color sits close to its tunic/trim (e.g. "guard"), the hem read as an undifferentiated second pair of legs. Fixed by narrowing `_HEM_FRACTION` to continue tapering past the waist instead of flaring back out, pinned by a test that measures the real generated art against the real leg geometry (`test_tunic_hem_does_not_read_as_a_second_pair_of_legs`) rather than eyeballing the constant. Tested: `test_character_view.gd` (+7 tests), `test_hero_sprite.gd` (one pre-existing assertion flipped to match the corrected, no-longer-flaring hem).
