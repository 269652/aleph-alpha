...
- **Wolves and sheep — forest's own named predator, and its prey** (medium) — ✅ Done — see `concept/ecosystem_dynamics.md`'s "Forest gets its own named predator" section. Forest's dominant predator slot had drawn from the anonymous `"predator"` placeholder (wolf-shaped, gray) rather than a real named species — the one gap left in the per-biome pattern desert/tundra/rainforest/mountain each already had (jackal/arctic_fox/jaguar/mountain_lion). Wolf is now a real `CreatureInfo` predator-role entry (own stats, `AnimalAnatomy` profile already existed unused), added to forest's `PREDATOR_SPECIES_POOL_BY_BIOME` alongside — not replacing — its existing lynx/predator/bear entries, and forest-exclusive (pinned by test: absent from every other biome's predator pool). Sheep is its (and deer's) real forest prey: an ordinary herbivore-role entry, forest-only, new `AnimalAnatomy` profile (woolly goat-shape variant, no horns). No new predation mechanism was needed — `is_predator` already means "hunts herbivore-role creatures generically" (see `CreatureInfo`'s own doc comment), so a wolf sharing forest with sheep and deer already hunts both by construction. Both get real illustrated art (`assets/sprites/animals/wolf.png`, `sheep.png`) rather than falling back to procedural silhouettes — the first illustrated-animal sheets supplied on a **chroma-keyed magenta** ground (real alpha channel, near-white cell dividers) rather than deer/boar/horse's white-background convention, so `IllustratedAnimalSprite` gained its own copy of `IllustratedStoneSprite`'s despill-before-slice/scrub-after-normalize chroma-key logic, gated per sheet behind a new `"magenta_keyed"` flag (deer/boar/horse are unaffected — the pass is skipped outright for them, not merely a no-op, pinned by test). Each sheet is a 2-row (walk, then eat) × 8-column grid with no dedicated idle row (idle synthesizes from eat's own frame 0, same as deer/boar); bands measured directly from the real PNGs, not eyeballed.
...
- **Snow fills in tile by tile, not the whole field at once** (medium) — ✅ Fixed — reported: "snow still covers a percentage of a whole chunk instantly instead of gradually filling individual tiles". One lying-snow depth still drives an entire snowfall (`Snowfall.accumulate` is unchanged — one clock, one number), but every tile now reshapes that SAME depth by its own deterministic warp exponent before banding it (`SnowLayer.tile_warp`, `pow(depth, warp)` — `0^k == 0` and `1^k == 1` for any positive k, so bare ground and a complete snowfall still land on the same band for every tile regardless of warp; only the MIDDLE of a snowfall differs tile to tile), grounded in real snow drifting unevenly with shelter/exposure rather than an even wash. Warp is spatially coherent (`PixelNoise.smooth`, patches ~7 tiles across), not tile-to-tile static, so cover reads as real drifting patches. `EarthChunkManager` replaced its old "repaint the WHOLE loaded field the moment one global band threshold crosses" trigger (`_snow_painted_band`, a single int) with a per-tile sweep (`_snow_painted_band_by_tile`) that recomputes every loaded tile's own band whenever the depth actually moves but only issues a real `set_cell` for tiles whose OWN band changed — cheap arithmetic against every loaded tile, engine calls only for the ones that actually flip, so the "thousands of set_cell calls" cost this file's own comments already flagged never regresses. Two related pre-existing gaps closed as part of the same rework: a chunk streamed in mid-snowfall now gets its snow immediately (`_load_chunk` sweeps its own tiles on load) rather than staying bare until the next field-wide threshold happened to cross; and an unloaded chunk's snow tiles are now actually erased from the snow `TileMapLayer` (`_unload_chunk`, matching the water/roof overlays right next to it) and forgotten from the per-tile bookkeeping (`_forget_snow_for_chunk`), where before neither ever happened. Verified against a real loaded field (not just `SnowLayer`'s own pure-function tests): a mid-snowfall depth shows multiple distinct bands across the loaded tiles, a complete snowfall (1.0) reaches the deepest band on every land tile, repeated calls at the same depth are idempotent, and moving away/back correctly forgets then immediately repaints.
...
✅ **Land/land diagonal corners -- the gap named above finally closed**
(reported directly: "diagonal blend border tiles [broken] at the border of
two biomes e.g. grass/forest"; see `docs/concept/terrain_borders.md`, a new
doc written for this whole system since it had never had one). Tracing the
report found the bug was actually WORSE than the "still an unblended hard
corner" note above described: `corner_direction_for`'s plain-notch branch
(`horizontal == vertical`) never actually excluded land/land pairs, so a
grassland cell notched by forest on two perpendicular sides already computed
a partner -- but `_corner_linear` routed ANY non-ocean `own_biome` into the
land-owning (concave, land-vs-OCEAN) family purely by `own_biome`'s own
ordinal, silently discarding `other_biome` entirely (that whole family
assumed `other_biome` was always `"ocean"`, since nothing had ever asked it
for anything else). A real grassland/forest corner therefore painted the
SAME atlas cell as grassland's own land-vs-ocean corner -- an ocean-shaped
rounded cutout showing up on dry land, not merely a hard corner. A THIRD
corner family now exists for land/land pairs, keyed by the UNORDERED pair
among `TerrainRenderer.LAND_BIOMES_BY_PRIORITY` (own_biome always the
lower-`BLEND_PRIORITY` side -- `corner_direction_for` is only ever reached by
that side, since `dominant_blend_for` already claims the higher-priority
side via ordinary cardinal dithering first) -- C(6,2) = 15 pairs, not
6×5 = 30 ordered ones, since only one direction of any land/land pair is
ever real. Separately, `corner_direction_for`'s MIXED-flanking-neighbor
branch (two different neighbors flanking a corner, carved toward whichever
wins `BLEND_PRIORITY`) had also always been gated to `biome_name == "ocean"`
only, even though the underlying dominance rule never actually depended on
ocean being involved -- a genuine three-biome land corner (no ocean at all)
...
earlier check by the time it was reached). `ATLAS_VERSION` bumped
(`_v20_land_land_corner_tiles`) so the new family's tiles actually reach a
real cached atlas rather than a stale pre-fix one being silently reused.
Verified with real baked atlas pixels, not just atlas coordinates (the same
"coordinates alone prove nothing about the pixels" lesson the pond-corner
bug above already taught): a real grassland/forest corner tile's carved
wedge reads as forest, its centre still reads as plain grassland -- compared
against each biome's own real rendered region rather than
`ProceduralTerrainSprite.BASE_COLORS`, since grassland and forest are both
illustrated today and an illustrated texture's real average colour can sit
meaningfully off its own flat-swatch constant (measured: illustrated
grassland's real average color sits closer to forest's OWN flat swatch than
to grassland's -- never a problem for the ocean-corner tests above, whose
ocean side stays procedural and close to its own swatch, but exactly the
kind of thing that makes two illustrated LAND biomes a genuinely different
