...
|---|---|---|---|
| Project Scaffold & Tile Rendering | ✅ Done | Godot 4.7 project set up; `scenes/world.gd`/`.tscn` is the root scene. `src/rendering/terrain_renderer.gd` no longer paints flat colors -- it bakes procedurally-generated, seeded pixel-art tiles (`procedural_terrain_sprite.gd`, speckled per-biome variants, animated-looking water) with `VARIANTS_PER_BIOME` variety per biome, plus a directional blend tile for every ordered pair of distinct biomes across every cardinal-direction subset: a border cell dithers toward whichever differing neighbor biome dominates on each edge it touches (`dominant_blend_for`/`generate_multi_directional_blend_image`), so "the forest side reads foresty, the grass side reads grassy" rather than a random 50/50 patchwork. Chunk seams blend too: `paint()` resolves out-of-chunk neighbors through `EarthChunkGenerator.biome_at_global` (the same deterministic per-tile function chunks are sliced from), so a water/sand/grass border falling on a chunk boundary merges exactly like an interior border. | small |
...
| Aggregate/Individual Promotion System | ✅ Done | `src/rendering/creature_renderer.gd` + `EarthChunkManager`: loading a chunk (i.e. player proximity, reusing the existing chunk-streaming radius) spawns individual creature nodes sized to that region's current aggregate population; unloading frees them. Each promoted creature has procedurally-generated pixel-art (`procedural_sprite_generator.gd`) and real per-individual AI (see Individual Creature AI below). Not replicated to multiplayer clients yet -- a connected client sees its own locally-seeded population, not the server's evolving one (see Multiplayer's known gaps). | large |
...
- **Tile placement/destruction (building system)** (medium) — ✅ Done — see Phase 3 table above; one placeable tile type, persisted across unload/reload.
...
- **Aggregate + individual-agent promotion simulation (open question)** (large) — ⬜ Not started
