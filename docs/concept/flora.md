...
Seed falls where the wind puts it (`WindDispersal`, weighted as the lightest
class in the world) — mostly close to the parent, occasionally a long way
downwind. Carrying it reliably further, against the wind and across ground
the wind would never take it, is what *animals* do, and that distinction is
the whole reason animals matter for spread. Where seed lands is not yet where
a plant stands: see
...
### How far apart flowers stand: escape from the parent

Reported live, twice: flowers *"spread or grow way too dense"*, and the seed
*"should be carried a bit further by the wind and birds so it leaves more
space between individual flowers"*.

The dispersal kernel was not the thing that was wrong. Real wind dispersal
genuinely is heavy tailed — most seed lands within a body-width or two of the
parent, which is exactly what `WindDispersal` already models and what
[seed_dispersal.md](seed_dispersal.md) is right to insist on. Flattening that
into a uniform scatter to buy spacing would trade a true mechanism for a
cosmetic one.

What was missing is the *other* half of the real process. **Where seed lands
and where a plant establishes are two different distributions.** Seed that
falls directly under its parent overwhelmingly dies: it is competing with an
established root system for the same water and light, it is sitting in the
highest concentration of that species' own pathogens, and it is exactly where
the seed predators that specialise on that plant are already looking. Only the
fraction that escapes the parent's neighbourhood becomes a plant. This is the
**Janzen–Connell** effect, and it is the standard explanation for why a
meadow reads as spaced-out individuals rather than as a mat around each parent
even though the seed rain is densest at the parent's foot.

So the model keeps the seed shadow honest and gates **establishment**:

- Seed still falls where the wind puts it, heavy tail and all. A drift of
  seed at a plant's foot is real and stays visible on the ground.
- A seed can only **root** into a plant if no established flower stands within
  `FlowerEstablishment.MIN_SPACING_TILES` of it. Closer than that, it is
  shaded and outcompeted, and it dies as a seedling.
- The rule is one rule, applied at every point a flower comes into being:
  the baked meadow, rain rooting lying seed (`FlowerPatch.root_seeds`), and an
  animal dropping carried seed (`FlowerPatch.plant`). A spacing rule that some
  paths could route around would silently refill exactly the gaps it opened.

The spacing is a **minimum**, not a lattice: it says how close two plants may
stand, and says nothing about how far apart they end up. Clumps still form —
seed still concentrates downwind of its parent, so a meadow is patches with
bare ground between them, not an even sprinkle. That distinction is what keeps
this from regressing pollinator foraging, which needs a *local* density of
blooms to work a circuit through (see
[Trap-lining](#trap-lining-a-forager-works-a-circuit-not-a-flower)); what it
removes is flowers standing shoulder to shoulder, not flowers being near each
other.

### The meadow you arrive to is what the wind already did

The initial meadow in a freshly generated chunk used to be an independent coin
flip per grassland cell (`SEED_CHANCE`, ~3.5%). That is not what a meadow is.
Two neighbouring cells both flowering was as likely as any other pair, so the
result was a uniform speckle at a density set by one constant — and being an
independent roll, it produced adjacent pairs and triples constantly. It also
had no relationship whatever to the wind, so a world whose entire dispersal
model is "light seed goes downwind" nevertheless *started* isotropic, and only
began to look windswept after a player stood and watched a chunk for a long
time.

The baked meadow is instead generated by **running the same dispersal the
live world runs**, from founder plants, under the region's prevailing wind:

- **Founders** are sparse, on a coarse world-space grid with per-cell jitter
  so they never sit on a visible lattice. World-space, not chunk-space: a
  founder near a chunk edge seeds *across* the boundary, so chunks agree at
  their seams instead of each meadow stopping dead at a chunk line.
- Each founder sheds through `WindDispersal.landing_offset` with the same
  weight class (`WEIGHT_FLOWER_SEED`) the live plants use, under the region's
  **prevailing wind** (`WeatherModel.prevailing_wind_direction`) — the
  long-run wind the landscape was shaped by, not one particular day's.
- Landings shed again for a further generation, so the meadow is a *lineage*
  spreading downwind rather than a single scatter around each founder. This
  is what actually produces streaks: the offspring of a plant that already
  travelled downwind starts from further downwind again.
- Every landing passes the same `FlowerEstablishment` spacing gate as a
  live-rooted seed, in world space, so the gate cannot behave differently on
  either side of a chunk boundary.

The visible result is what the wind should have produced: sparse plants,
clumped into patches, with the patches drawn out along the prevailing wind
and bare ground between them. Turn the prevailing wind around and the same
seed grows the mirrored meadow — which is the test that this is wind being
simulated rather than a random offset wearing wind's name.

Two consequences worth naming:

- The dispersal kernel is now load-bearing for **worldgen**, not just for the
  slow live spread. A change to `WindDispersal`'s tail now changes what a
  player sees the instant they walk into a new chunk.
- `FlowerPatch.set_wind` had **no production caller at all** — the live shed
  path ran permanently at `strength = 0.0`, i.e. dead calm, in every real
  game, so live-shed seed only ever got `CALM_SCATTER_TILES` and the downwind
  drift the model computes was never once applied outside its own tests. The
  weather now reaches the patch, so live shedding drifts downwind too.
