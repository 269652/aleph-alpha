...
- ✅ `MushroomMarker` (`src/rendering/mushroom_marker.gd`) — the visible,
  ...
  the same for `bitten_frame_for`, with no corpse involved -- delivered
  for 4 species so far (black_trumpet/champignon/chanterelle/
  false_death_cap), see "Bitten by a decomposer". `take_mushroom_bite()`
  -- its own method, deliberately not `take_bite` -- is what
  `DecomposerMarker`'s bite path calls.
