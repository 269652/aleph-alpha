...
This doc originated on an unmerged exploratory branch (`claude/wooden-club-
sprite-7e5fsn`) as a proposed replacement for that branch's OWN two-row
`wooden_club_combat.png` pilot, and sat unresolved for several days —
neither shape had actually shipped. Brought to `main` and adopted
(2026-09-07) as the convention new item/structure art follows going
forward, explicitly instead of the two-row shape (see
[item_illustrations.md](item_illustrations.md)'s own "Combat sheets"
section, which now cross-references here).

- ✅ `illustrated_art_registry.gd` — subject declarations (`wooden_club`,
  `campfire`, matching this doc's own two worked examples below), anchors,
  timing, overlays, `chroma_key`. **Not yet included**: prompt fragments
  (waits on `tools/print_art_prompts.gd`, still ⬜ below), and the
  "every file under `assets/sprites/<subject>/` is addressable" sweep test
  this doc originally specified — there is no real art under this
...
- ✅ `illustrated_art_resolver.gd` — the lattice above, returning
  ...
  **Implemented as an actual lattice search** (try every combination of
  {requested, relaxed} across the 4 axes, fewest axes relaxed first, ties
  broken by a fixed season/state/context/animation priority), NOT a
  straight walk of the numbered fallback-order list above, because that
  list and this doc's own wooden-club worked example below disagree: the
  worked example requires state to be preferred over animation when both
  a state-only and an animation-only relaxation would independently
  resolve (`worn/attack` → `pristine/attack`, not `worn/still`), which the
  numbered list (animation listed first) does not produce. The resolver
  follows the worked example; see its own doc comment for the full
  reasoning, and its "Also NOT implemented" notes for two further
  resolved ambiguities (the icon-falls-back-to-held aside directly below
  contradicts rule 4's own stated direction and is not implemented; a
  non-seasonal context's season is not yet auto-normalized). 23 tests
  (`test_illustrated_art_registry.gd`, `test_illustrated_art_resolver.gd`)
