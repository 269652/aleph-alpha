...
| `VOLUME_QUANTUM_CM3 = 0.05` | A balance reads to the gram, so a volume difference that cannot move a scale must not mint an id. Derived from `MaterialProperties`' own densities rather than restated: 0.05 cm³ of the densest material modelled (gold, 19.30 g/cm³) is 0.97 g, and ten quanta of it would be plainly perceptible — so this is the *coarsest* quantum that still hides under the threshold, not an arbitrarily fine one. The test asserts that physical invariant and reads the roster live; it deliberately does **not** assert *which* material is densest. That indifference has now paid off **twice**: the densest material went iron → copper (the alloy pass) with no change needed, then copper → gold (the 2026-08-28 material pass), which genuinely *did* break the bound and forced this re-derivation 0.1 → 0.05 — exactly the guard firing as its own doc comment predicted it would. | `test_the_volume_quantum_is_finer_than_a_balance_can_read`, `test_the_volume_quantum_is_no_finer_than_it_needs_to_be` |
