...
| `VOLUME_QUANTUM_CM3 = 0.1` | A balance reads to the gram, so a volume difference that cannot move a scale must not mint an id. Derived from `MaterialProperties`' own densities rather than restated: 0.1 cm³ of the densest material modelled (copper, 8.96 g/cm³) is 0.90 g, and ten quanta of it would be plainly perceptible — so this is the *coarsest* quantum that still hides under the threshold, not an arbitrarily fine one. The test asserts that physical invariant and reads the roster live; it deliberately does **not** assert *which* material is densest, which already changed once (iron until copper arrived with the alloy work) without the quantum needing to move. | `test_the_volume_quantum_is_finer_than_a_balance_can_read`, `test_the_volume_quantum_is_no_finer_than_it_needs_to_be` |
