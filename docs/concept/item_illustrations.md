...
|---|---|---|
| **Icon** | Inventory grid, hotbar, paperdoll equip slots, drag-preview, tooltip | Eventually, per item — but see `sprite_id` below; nothing blocks on this today. Full catalog scaffolded into 11 generation-ready kits in [ai_sprite_prompts.md §9](../art/ai_sprite_prompts.md#9-general-item-icons--one-kit-per-visual-archetype-2026-08-28). As of 2026-09-08, the first 100 catalog ids also have a bare `icon` context declared directly in [illustrated_art_addressing.md](illustrated_art_addressing.md)'s registry (see that doc's own Status list) — the two scaffolds are complementary, not duplicate: `ai_sprite_prompts.md` §9 is the human-readable generation-prompt text, the registry is the runtime addressing data a real caller resolves against; neither authors actual pixels yet. |
