...
# -- one purse, and it is the one the merchant really pays into -----------
#
# _fund_village_as_a_merchant_would above deposits into the VillageMarket,
# which is where the wage is drawn from -- so this whole suite passed while
# the real wiring did something else. EarthChunkManager._step_merchant_
# visits pays NpcEconomy.deposit_to_purse(market, ...) where `market` is
# the settlement's PERSISTED Market (MarketStore.market_for), while
# _draw_subsistence_wage reads purse_of(self.market), the VillageMarket.
#
# PURSE_META is set on whichever market OBJECT is in hand, so those are two
# tanks sharing one name: a village can be paid and still starve beside its
# own stall. Measured (tools/probe_village_purse.gd) on a real village --
# both read 0.0, which is why this was not the cause of the famine, only
# the next thing that would have been.
#
# The purse belongs on the persisted Market. A VillageMarket is rebuilt
# from scratch on every chunk load, so a purse kept there dies with the
# chunk -- the identical defect bind_household_wallet exists to fix for a
# villager's own wallet ("a villager's whole working life evaporated the
# moment the player walked away").

const Market = preload("res://src/emergence/market.gd")


## The disconnect itself: gold paid where a merchant really pays it must
## feed the villager who is really hungry.
func test_a_villager_draws_the_wage_the_merchant_really_paid():
	var settlement_market := Market.new()
	market.add_stock("meat", 40.0)
	assert_true(market.can_buy_meal(), "precondition: there is food on the stall")
	NpcEconomy.deposit_to_purse(settlement_market, 100.0)

	var blacksmith := _economy("blacksmith")
	blacksmith.bind_settlement_purse(settlement_market)
	blacksmith.needs.advance(100000.0)
	assert_eq(blacksmith.wallet.balance, 0, "precondition: they own nothing")

	blacksmith.step(0.01, false, world, Vector2.ZERO)

	assert_false(
		blacksmith.needs.is_hungry(),
		"a village that was paid must be able to feed its own blacksmith"
	)
	assert_lt(
		NpcEconomy.purse_of(settlement_market), 100.0,
		"and the wage really came out of the purse the merchant filled"
	)


## Nothing is conjured: an unbound economy behaves exactly as it always
## did, so a bare NpcEconomy in a test or a village with no settlement
## record is not silently given a second treasury.
func test_an_unbound_economy_still_draws_from_its_own_market():
	_fund_village_as_a_merchant_would(market)
	var funded := NpcEconomy.purse_of(market)
	assert_gt(funded, 0.0, "precondition: the village market's own purse holds something")

	var blacksmith := _economy("blacksmith")
	blacksmith.needs.advance(100000.0)
	blacksmith.step(0.01, false, world, Vector2.ZERO)

	assert_false(blacksmith.needs.is_hungry())
	assert_lt(NpcEconomy.purse_of(market), funded, "drawn from the only purse it has")


## And a bound one does NOT also reach into the village market's meta --
## one purse means one, or the same coin is spendable twice.
func test_a_bound_economy_does_not_also_spend_the_village_markets_meta():
	var settlement_market := Market.new()
	market.add_stock("meat", 40.0)
	NpcEconomy.deposit_to_purse(market, 100.0)  # the WRONG tank, left untouched
	var blacksmith := _economy("blacksmith")
	blacksmith.bind_settlement_purse(settlement_market)
	blacksmith.needs.advance(100000.0)
	...
	assert_almost_eq(
		NpcEconomy.purse_of(market), 100.0, 0.0001,
		"a bound villager must not be able to spend the other tank too"
	)
	assert_true(blacksmith.needs.is_hungry(), "an empty settlement purse pays nobody")
