...
## Regression: EarthChunkManager.step_wild_mushrooms is a real, tested
## function (see test_earth_chunk_manager_mushrooms.gd) -- but nothing
## proves World's own per-frame ecology step actually calls it, the exact
## same "wired but never called" trap test_world_ecology_batch_wild_crops.gd
## already caught for wild crops (see that file's own header comment). A
## wild mushroom patch would therefore render on chunk load and then never
## advance, in a real play session.
##
...
## Proven the same way test_ecology_batch_advances_a_real_wild_crop_patchs_
## growth proves its own step ran: by observing a real effect only the
## step produces.
