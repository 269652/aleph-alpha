...
func test_the_drift_translation_is_bounded_by_the_noise_period():
	...
	# line_field's own cumulative-arc walk (see the "smear follows the
	# course's curve" work) reads three taps -- q itself plus a forward and
	# backward walk -- each of which must be the TILED noise, not plain
	# value_noise, or this same field would still shred at the same TIME
	# this test guards against.
	assert_true(
		RiverFlowShader.SHADER_CODE.contains("value_noise_tiled(q, drift_period) * 5.0"),
		"line_field's centre tap must sample the noise that tiles at drift_period"
	)
	assert_true(
		RiverFlowShader.SHADER_CODE.contains(
			"value_noise_tiled(forward, drift_period) + value_noise_tiled(backward, drift_period)"
		),
		"line_field's forward/backward arc taps must sample the noise that tiles at drift_period"
	)
