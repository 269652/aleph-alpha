...
## --- Fresh water (docs/concept/hydrology.md "Water kinds") ---


func test_ocean_lake_and_river_are_water():
	for biome_name in ["ocean", "lake", "river"]:
		assert_true(BiomeClassifier.is_water(biome_name), "%s is water" % biome_name)


func test_land_biomes_are_not_water():
	for biome_name in ["mountain", "tundra", "forest", "grassland", "rainforest", "desert", ""]:
		assert_false(BiomeClassifier.is_water(biome_name), "%s is not water" % biome_name)


func test_only_lake_and_river_are_fresh_water():
	assert_true(BiomeClassifier.is_fresh_water("lake"))
	assert_true(BiomeClassifier.is_fresh_water("river"))
	assert_false(BiomeClassifier.is_fresh_water("ocean"))
	assert_false(BiomeClassifier.is_fresh_water("grassland"))


func test_known_biomes_stay_the_land_and_ocean_vocabulary():
	# Fresh water is a separate list: every renderer/spawn table keyed on
	# KNOWN_BIOMES keeps working unchanged (hydrology.md phase 1).
	assert_false(BiomeClassifier.KNOWN_BIOMES.has("lake"))
	assert_false(BiomeClassifier.KNOWN_BIOMES.has("river"))
	assert_true(BiomeClassifier.ALL_BIOMES.has("lake"))
	assert_true(BiomeClassifier.ALL_BIOMES.has("river"))
	for biome_name in BiomeClassifier.KNOWN_BIOMES:
		assert_true(BiomeClassifier.ALL_BIOMES.has(biome_name))


func test_a_water_kind_from_hydrology_wins_over_every_land_check():
	var slope := BiomeClassifier.SLOPE_NOT_PROVIDED
	assert_eq(classifier.classify(0.5, 0.9, 0.1, 0.3, 0.85, slope, "lake"), "lake")
	assert_eq(classifier.classify(0.5, 0.1, 0.9, 0.3, 0.85, slope, "river"), "river")
	# Even a slope that would otherwise force mountain.
	assert_eq(classifier.classify(0.5, 0.5, 0.5, 0.3, 0.85, 89.0, "river"), "river")


func test_below_sea_level_is_ocean_even_with_a_water_kind():
	assert_eq(classifier.classify(0.1, 0.5, 0.5, 0.3, 0.85, BiomeClassifier.SLOPE_NOT_PROVIDED, "lake"), "ocean")


func test_an_empty_water_kind_changes_nothing():
	assert_eq(classifier.classify(0.5, 0.5, 0.7, 0.3, 0.85, BiomeClassifier.SLOPE_NOT_PROVIDED, ""), "forest")


func test_dominant_biome_breaks_ties_with_fresh_water_deterministically():
	var tied := PackedStringArray(["lake", "forest"])
	var reversed := PackedStringArray(["forest", "lake"])
	assert_eq(classifier.dominant_biome(tied), classifier.dominant_biome(reversed))
	assert_eq(classifier.dominant_biome(tied), "forest", "land vocabulary orders ahead of fresh water")
