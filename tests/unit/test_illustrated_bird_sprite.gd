...
# -- _keyed_image performance ---------------------------------------------
#
# _keyed_image was a hand-written duplicate of SpriteSheetSlicer.chroma_keyed
# -- same per-channel tolerance test, same "matched pixels become fully
# transparent" result -- but written as a plain per-pixel get_pixel/set_pixel
# double loop, the exact naive technique chroma_keyed itself was already
# fixed for (see its own doc comment for the measured three-way comparison,
# and docs/progress.md's "Still at 1fps" investigation). Nothing about that
# fix reached this file's own copy.
#
# It lands in the character creator's first open: its diorama spawns a few
# songbirds, and each SPECIES pays its own sheet load (~420-460ms measured --
# see docs/concept/character_creator_preview_scene.md's "Load cost" table).


## Real sheet resolution (1536x1024 -- see _SHEETS' own doc comment), format
## RGB8. Uniform non-key fill: this pass has no early-out or bounding box, so
## every pixel costs the same fixed handful of comparisons and a solid fill
## is a faithful worst case (mirrors test_illustrated_terrain_sprite.gd's and
## test_illustrated_stone_sprite.gd's own identical budgeted pins).
func test_keying_a_sheet_completes_quickly_at_real_sheet_resolution():
	var width := 1536
	var height := 1024
	var image := Image.create(width, height, false, Image.FORMAT_RGB8)
	image.fill(Color(0.3, 0.5, 0.2))
	var start_usec := Time.get_ticks_usec()
	IllustratedBirdSprite.key_out_background(image)
	var elapsed_ms := (Time.get_ticks_usec() - start_usec) / 1000.0
	assert_lt(
		elapsed_ms,
		200.0,
		(
			"keying one %dx%d sheet took %.0fms -- a naive per-pixel get_pixel/set_pixel loop regressed back in"
			% [width, height, elapsed_ms]
		)
	)


## Speed is worthless if the pixels change: a pixel within CHROMA_KEY_
## TOLERANCE of the key goes fully transparent, everything else is untouched.
func test_keying_clears_the_key_colour_and_leaves_everything_else_alone():
	var image := Image.create(2, 1, false, Image.FORMAT_RGB8)
	image.set_pixel(0, 0, IllustratedBirdSprite.CHROMA_KEY)
	image.set_pixel(1, 0, Color(0.3, 0.5, 0.2))

	var keyed: Image = IllustratedBirdSprite.key_out_background(image)

	assert_almost_eq(keyed.get_pixel(0, 0).a, 0.0, 0.01, "the key colour must go fully transparent")
	var kept := keyed.get_pixel(1, 0)
	assert_almost_eq(kept.a, 1.0, 0.01, "real bird content stays opaque")
	assert_almost_eq(kept.r, 0.3, 0.01)
	assert_almost_eq(kept.g, 0.5, 0.01)
	assert_almost_eq(kept.b, 0.2, 0.01)
