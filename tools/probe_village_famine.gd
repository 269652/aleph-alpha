...
func _purse_and_pockets(chunk_coord: Vector2i, settlement_id: String) -> String:
	...
	var SettlementFood = load("res://src/emergence/settlement_food.gd")
	var village_market = SettlementFood.village_market_for(settlement_id, _manager._loaded_villages)
	# The purse a wage is really drawn from: the LIVE VillageMarket, which
	# is where _step_merchant_visits pays. The persisted emergence Market
	# carries a purse of its own and nobody spends it, so both are printed
	# -- a village earning into the wrong tank looks exactly like a village
	# nobody ever paid.
	var settlement_purse: float = 0.0 if village_market == null else NpcEconomy.purse_of(village_market)
	var traded = _manager._market_store.market_for(settlement_id)
	var stale: float = 0.0 if traded == null else NpcEconomy.purse_of(traded)
	var broke := 0
	var villagers := 0
	for node in _manager._loaded_villages.get(chunk_coord, []):
		if not is_instance_valid(node) or not node.has_method("setup_economy"):
