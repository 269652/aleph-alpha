...
##    roots still planted at the same ground line and no seam or stretch
##    visible at the mesh's own subdivision boundaries.
...
const VIEW := Vector2i(160, 80)
const OLD_GROUND := Vector2(40, 68)
const NEW_GROUND := Vector2(120, 68)
...
func _init() -> void:
	...
	for push_label in ["calm", "walker"]:
		...
		img.save_png(path)
		print("saved ", path, "  (left = old sampling-only bend, right = geometry bend)")
