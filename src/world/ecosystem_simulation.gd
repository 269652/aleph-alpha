...
## Predator-eats-herbivore kills and the player's own weapon both route through
## CreatureMarker.take_damage into _die(), which is what calls this.
##
...
## has a stake in), so a barn losing a sheep is not the land losing one. That
## exemption lives at the call site, in CreatureMarker._book_death_against_the_region.
