...
## The upper storey of a two-story house (docs/concept/housing.md's "Two-story houses" section), keyed by local tile coordinate like `modifications` -- its OWN layer for the same real reason roof_modifications/furniture_modifications already need theirs: a two-story house's upper floor occupies the SAME (x, y) cells as its ground floor (a real stacked layer, not a second footprint elsewhere), so `modifications` (which can only ever hold one tile id per cell) cannot also hold it. Painted onto its own TileMapLayer (see EarthChunkManager.set_upper_floor_layer), visible only while a player is NOT standing inside the corresponding upper-floor room (the SAME real room-hiding rule roofs already use, one layer up) -- which is what makes its own real windows visible from outside at ground level.
var upper_floor_modifications: Dictionary = {}

## Furniture on the upper storey (docs/concept/housing.md's "Interior furniture" section) -- reported directly alongside two-story houses themselves ("no room decoration"), for BOTH floors, not the ground floor alone. Its OWN layer for the SAME reason upper_floor_modifications needs one one layer down: an upper-floor table sits at the SAME (x, y) as the upper floor's own real wall/window/floor piece there, and one dict can only ever hold one id per cell. Painted onto its own TileMapLayer (see EarthChunkManager.set_upper_floor_furniture_layer), hidden in lockstep with upper_floor_modifications' own real cells (not painted unconditionally the way ground furniture_modifications is) -- ground furniture is never hidden by anything because nothing ever occludes the ground layer's own room from a bird's-eye view; the upper floor's own room genuinely IS hidden while a player stands inside it (see _update_upper_floor_visibility), so its furniture must hide in step or it would float visibly over bare ground with no walls around it.
var upper_floor_furniture_modifications: Dictionary = {}
