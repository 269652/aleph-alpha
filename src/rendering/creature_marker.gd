...
func _wander_step(delta: float) -> void:
	...
	var candidate := _wander.step_position(home, position, _elapsed_time, delta, wander_seed, _wander_radius())
...
## WorldItemBus, so world.gd spawns the ground items), reports the death to
## the region's aggregate population (see EarthChunkManager.record_death_at
## / EcosystemSimulation.record_death -- this is where a kill is actually
## finalized, so it covers both predator-eats-herbivore kills and player
## weapon kills, since both route through here), and frees this marker.
