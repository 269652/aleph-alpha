...
## Real illustrated art for the millipede's four animations (see
## docs/concept/soil_fauna.md "Millipedes: a dedicated autumn leaf-litter
## decomposer"). Same "hand-drawn sheet -> SpriteSheetSlicer -> cached
## frames" shape as IllustratedCaterpillarSprite/IllustratedWormSprite,
## deliberately mirrored rather than built on IllustratedAnimalSprite/
## IllustratedDecomposerSprite: a millipede carries no CreatureMarker/
## AnimalAnatomy stack, and -- like the worm/caterpillar -- there is only
## ever one kind of millipede in this game, so this skips the species
## dimension those two both carry and keys everything by action alone.
##
## millipede.png is a perfectly regular 8-column x 4-row grid (1536x1024,
## 192x256 per cell -- dimension-identical to worm.png/caterpillar.png,
## confirmed directly against the PNG header), sliced from that KNOWN
## fixed grid rather than through SpriteSheetSlicer.detect_frames'
## content-gap heuristic, the same reasoning IllustratedWormSprite/
## IllustratedCaterpillarSprite's own doc comments give.
##   row 1 "crawl" -- a flat, level, many-legged gait: the travel/wander
##     pose, and what approaching a leaf plays.
##   row 2 "alert" -- rears its front segments up, head raised: played
##     while EATING, a millipede pausing to feed.
##   row 3 "curl" -- rolls into a defensive coil, a real millipede threat
##     response. Not yet wired to any trigger (see the concept doc's own
##     "What this does NOT include").
##   row 4 "crushed" -- a level crawl transitioning into a flattened,
##     splattered pose. Not yet played as an animation -- a crushed
##     millipede is removed instantly, the same "just disappear" outcome
##     a crushed worm/caterpillar already has (see the concept doc).

const SpriteSheetSlicer = preload("res://src/rendering/sprite_sheet_slicer.gd")
const SpriteSheetLoader = preload("res://src/rendering/sprite_sheet_loader.gd")

const _SHEET_PATH := "res://assets/sprites/animals/millipede.png"
const _COLUMNS := 8
...
## Sprite/IllustratedWormSprite's own shared-canvas convention exactly (same
## source cell aspect ratio, so the same canvas/baseline numbers apply
## unchanged).
...
## IllustratedCaterpillarSprite.WORLD_LENGTH_TILES (0.32) -- bounded rather
## than exact-matched against prior art, since there is no earlier
## ProceduralMillipedeSprite to preserve continuity with (this is a brand
...
## Keyed by action, shared across instances -- every millipede drawing the
...
## genuinely different-sized species/poses): a millipede is the same
## creature at the same real length whether it's crawling, alert, curled or
## crushed -- normalize_frames already preserves each row's own relative
...
## gait is the one action guaranteed to represent the millipede's own
## resting length, unlike "alert"'s reared-up peak or "curl"'s coiled
## silhouette.
