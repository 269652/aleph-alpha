...
## Loads a sheet off disk. Delegates the actual read to SpriteSheetLoader
## (prefers the imported resource, falls back to reading the file directly
## so the sheets work in a headless test run where Godot's import step has
## not necessarily happened) -- this wrapper only adds the existence guard
## (a species may have no sheet at this path at all, which is not an error)
## and the per-path cache/RGBA8-normalization every caller here relies on.
