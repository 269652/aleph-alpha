...
## How often the hover scan re-runs while the mouse is NOT moving (FPS
## regression round 10, docs/concept/soil_fauna.md). _update_hover_tooltip
## walks every hoverable node in the world -- 24 marker classes, thousands
## of nodes, one distance check each -- measured live at ~6.7ms per call,
## and HOVER_REFRESH_INTERVAL is a wall-clock cadence, so at 4 fps it ran on
## every single frame. The scan only has anything new to report when the
## mouse has moved, or when something may have walked under a still mouse:
## the first is caught on the ~30 Hz cadence as before, the second now only
## at this rate. Pinned by test_world_hover_tooltip_throttle.gd.
const HOVER_IDLE_REFRESH_INTERVAL := 0.25
var _hover_idle_accumulator := 0.0
## Where the mouse was at the last hover scan; INF until one has happened,
## so the very first scan is never mistaken for "already up to date".
var _last_hover_scan_mouse := Vector2(INF, INF)
...
func _ready() -> void:
	...
	# render until the ~80s heavy setup below had already finished), then
	# moved to here at the top and awaited in full (bug: correct in
	# isolation -- a live, timestamped launch confirmed the intro really did
	# play its full ~3.2s -- but running it before the heavy setup means it
	# still shares this machine's real boot experience with that setup's
	# own long, single-threaded synchronous block, which Windows itself
	# marks the whole window "Not Responding" (a grey window) for the
	# entire time it runs -- always true, on this machine, regardless of
	# the intro; measured 80+ real seconds on 2026-09-07, ~51-54s on later
	# runs). A 3.2s animation squeezed in just before a minute-long grey,
	# hung-looking window reads as "nothing played" even though it
	# genuinely did -- reported a third time, directly: "it hangs for a
	# minute or two when starting and just shows a grey window."
	#
	# `args`/`plain_interactive_launch` are still computed here because the
	# solo/server/join dispatch further down needs them, and because the
	# license/GitHub-identity checks above must stay exactly where they are.
	...
	if "--server" in args:
		...
		# The heavy setup above is done and the engine is fully responsive
		# again -- exactly the moment the boot logo intro is now deliberately
		# saved for (see _ready()'s own comment on why it no longer runs
		# earlier). Show the menu FIRST, so it is already fully built and
		# would be sitting there interactive the instant the intro is
		# skipped or finishes, then play the intro as a bumper on top of
		# it -- reversed from how this read before (intro then menu): the
		# menu no longer waits on anything.
		_show_main_menu()
...
## Plays the boot logo intro (see IntroSplash, docs/concept/intro_splash.md)
## as a full-screen bumper and AWAITS its own `finished` signal -- on
## completion or an early skip alike -- before returning, so a caller that
## awaits this genuinely cannot proceed past it until the intro is done.
## Frees the intro itself once finished; nothing about it persists past this
## call, unlike an earlier pass that kept it alive across the whole heavy
## per-boot setup (see _ready()'s own comment on why that placement was
## superseded). Two call sites now, both deliberately AFTER whatever they're
## a bumper for is already visible/underway, never before or during it:
## _show_main_menu() (once the menu itself is already built and interactive
## underneath) and _on_menu_start_requested() (once "New Game"/"Host Game"
...
## Whether the hover scan is due this frame: on the HOVER_REFRESH_INTERVAL
## cadence, and then only if the mouse has moved since the last scan or
## HOVER_IDLE_REFRESH_INTERVAL has passed (see that constant). Pure state
## machine over its own accumulators, so it is testable without a tree
## (test_world_hover_tooltip_throttle.gd); the caller passes the mouse in.
func _hover_rescan_due(delta: float, mouse: Vector2) -> bool:
	_hover_accumulator += delta
	_hover_idle_accumulator += delta
	if _hover_accumulator < HOVER_REFRESH_INTERVAL:
		return false
	_hover_accumulator = 0.0
	if mouse == _last_hover_scan_mouse and _hover_idle_accumulator < HOVER_IDLE_REFRESH_INTERVAL:
		return false
	_last_hover_scan_mouse = mouse
	_hover_idle_accumulator = 0.0
	return true
...
func _client_process(delta: float) -> void:
	...
	# "Generalized to bugs too" follow-ups) -- mirrors the tread_snow_at
	# pair just above exactly (player, then every creature), but keyed on
	# real weight rather than snow depth, so it runs regardless of season.
	# crush_worm_at/crush_caterpillars_near/crush_millipedes_near/
	# crush_ants_near/crush_decomposers_near share the identical momentum
	# value per stepper -- one shared physics rule (CrushMechanic), two
	# detection shapes (a worm is per-tile cell state, a caterpillar/
	# millipede/ant forager/decomposer a real Node2D). No debounce needed
	# for any of the five: every removal is already idempotent
	...
	# mushrooms.md's own Status note on the reversal): crush_mushroom_at's
	# bool return feeds Karma exactly like every crush call above. A
	...
	for creature in loaded_creature_markers:
		...
		# moved or not, measured live at ~26ms a frame -- the single largest
		# cost inside World's own _process. A creature only crushes what is
		# under a tile it has just moved onto, so one still standing on the
		# tile it last crushed from skips every scan -- the same per-stepper
		# "last tile" debounce _last_scar_step_tile gives the player's own
		# path scarring. (The idempotency argument in the block comment above
		# still holds; it just never made the redundant scans free.)
		var creature_tile := _tile_for_position(marker.position)
		if creature_tile == marker.last_crush_step_tile:
			continue
		marker.last_crush_step_tile = creature_tile
