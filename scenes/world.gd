...
func _client_process(delta: float) -> void:
	...
	# too"/"Generalized to millipedes too" follow-ups) -- mirrors the
	# tread_snow_at pair just above exactly (player, then every creature),
	# but keyed on real weight rather than snow depth, so it runs regardless
	# of season. crush_worm_at/crush_caterpillars_near/crush_millipedes_near
	# share the identical momentum value per stepper -- one shared physics
	# rule (CrushMechanic), two detection shapes (a worm is per-tile cell
	# state, a caterpillar or millipede a real Node2D). No debounce needed
	# for any of the three: every removal is already idempotent
	...
	# the two loops below did the crushing. A crushed millipede charges the
	# same constant -- its name predates this third species, but the event
	# it represents is identical (see karma.gd's own doc comment).
	...
	if _chunk_manager.crush_millipedes_near(local_player.position, _PLAYER_STEP_MOMENTUM_KG_M_S):
		local_player.apply_karma_delta(-Karma.WORM_OR_CATERPILLAR_CRUSH_PENALTY)
	# A mushroom is a fungus and a walnut a plant seed, neither an animal --
	# crush_mushroom_at/crush_walnut_near's own bool returns are
	# deliberately not fed into Karma the way the worm/caterpillar/
	# millipede calls just above are (see docs/concept/mushrooms.md,
